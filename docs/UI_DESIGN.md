...
**Historical bug, fixed in the Pixel-Exact UI cycle (`implementation_plan.md`,
items PX-1/PX-2) — kept here because the reasoning still governs how geometry
is authored.** `project.godot` used to set
...
never changes at runtime — this is a second, independent reason PX-2 chose
...
(PX-4) — it is a rate over a spatial unit, so it has to track the same scale
...
(PX-4) — `COMMAND_WIDTH`, `SPELL_WIDTH`, `PROMPT_WIDTH`, `FORECAST_WIDTH`,
...
every 2D panel to a shared 1152 × 648 logical base (PX-2 replaced that with
...
was a real, pre-existing bug, found while re-measuring for PX-4 and fixed
there.** Measured against the shipping font at the *current* x2 scale, with
...
hypothetical worst cases. This predates PX-4 entirely —
