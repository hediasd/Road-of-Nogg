...
A sheet can instead be packed as a honeycomb by adding `"LAYOUT": "honeycomb"` to its config. Hexes then interlock as they do on the map (column step 24 px, alternate columns 16 px apart), so neighbours share edges and transitions can be painted across a seam. Every border is **outward**: odd columns are the long ones and stick out past their even neighbours at both the top and the bottom, so even columns have no row 0. Hex (c, r) has its top-left corner at `(24·c, 32·r)` for odd c and `(24·c, 32·r − 16)` for even c. A sheet with C columns and R rows is `24·(C−1)+32` × `32·R` px; use an odd C so both side columns are the short kind. The catalog unpacks it into one masked 32 × 32 frame per hex on load, so `CELL` is still (column, row) and hashes match a grid sheet holding the same pixels. The editor's picker shows a honeycomb sheet as authored and selects by hex. `temp2_hex32_starter_v2-Recovered-export` is the first honeycomb sheet (5 × 5, 22 hexes); its grey `placeholder` hexes fill slots with no art yet.
