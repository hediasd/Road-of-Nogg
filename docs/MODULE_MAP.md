...
| `src/presentation/effects/` | Transient visual effects | `VfxPlayback` (the contract), `SpellVfxCatalog`, `ElementalCubeRitualEffect`/`ElementalCubeRitualProfile`, parked legacy spell effects, `VfxTextures`, `DamageNumberBillboard` | Godot 3D API | simulation | [`VFX_DESIGN.md`](./VFX_DESIGN.md) |
