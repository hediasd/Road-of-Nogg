...
| `src/battle_sim/` | Canonical runtime: state, turn order, resolvers, setup, replay, event bus, adapter port | `BattleSimulator`, `BattleState`, `BattleSetupConfig`, `BattleSetupFactory`, `BattleCommand`, `BattleEvents`, `IBattleVisualAdapter`, `BattleStateSerializer`, `BattleReplayRunner` | every headless directory above | `src/presentation/`, `src/systems/`, scene tree | [`ARCHITECTURE.md`](./ARCHITECTURE.md) |
