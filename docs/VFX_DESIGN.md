...
**Resolved by v2, below: the answer was to cut them.** v2 kept the flares on
the hypothesis that spin would rescue them, and measured that it does not.

### Technique charge aura v2: spin, blades, and a dispersing release

`technique_charge_aura_v2` is a second debug-catalog entry, forked from v1 on
2026-08-29 under §4's sibling rule and owning its own profile, effect, shader
and copy of the mask. v1 is untouched and both are previewable side by side.
Neither is wired to a spell; that is still the `BACKLOG_LONGTERM.md` item.
