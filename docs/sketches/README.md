...
| [2026-09-20-unit-shades-ready-moved-spent.html](./2026-09-20-unit-shades-ready-moved-spent.html) | Hex battle unit shades | That a unit carries three shades and not two -- ready, moved (still commandable), spent (inspection only) -- and that both darkened ones are chosen in *perceived* lightness, because `unit_darken` multiplies linear albedo and a 0.72 multiplier removes only 44% of the brightness a player sees. Carries the curve that correction came from, the ladder the two constants are read off, and the rejected luminance-under-slate treatment reconstructed from its own formula beside its replacement, on the two units it hurt most. |
