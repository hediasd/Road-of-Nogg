...
| [2026-09-12-hex-editor-authoring-workspace.html](./2026-09-12-hex-editor-authoring-workspace.html) | Hex editor workspace | That the hex editor is a workspace and not a debug scene: art chosen from a tilesheet on the left, the map in the middle with nothing drawn over it, layers on the right, and a first map that starts from **New** rather than from whichever region the explorer was showing. Carries the live sheet the picker's selection rules were written against -- plain click, Ctrl-click, Shift-range, Escape to clear. Annotated on acceptance with the two places the shipped editor went further and why: the tools moved off the toolbar into a pinned menu under the tilesheet, because one row cannot hold document commands and map commands at 1280x720 without an author having to read it to tell which is which; and the stage stops at the document's own edge at native resolution, which a flat canvas mock has no way to get wrong. |
| [2026-09-22-side-turn-restore-and-frame-cost.html](./2026-09-22-side-turn-restore-and-frame-cost.html) | Side-turn restore, and what a frame costs | Two rendered frames across one `restoreSideTurn()`, which settled whether a restored side turn should unwind what the screen had drawn or throw it away and redraw. Before: a spell mid-flight, a damage number rising, a selection hex, an area dome and a movement trail, all belonging to the timeline about to be abandoned. After: a clean authoritative board, nothing half-faded and nothing stranded. Reversal would have obliged every future effect to define an inverse, and the first one to forget would leave the board lying. Carries the frame-cost measurement beside it, including the case it does **not** fix: a single enumeration slice can still overshoot a frame budget on its own, which is the remaining 17-20 ms tail. |
| [2026-09-24-cube-spell-motion-studies.html](./2026-09-24-cube-spell-motion-studies.html) | Cube-only placeholder spell vocabulary | Twenty-four scrubbable placeholder effects across travel, impact, control, and restoration. The cube populations, relative sizes, trajectories, and normalized three-beat timing shown here are the implementation target; the neutral caster and target cubes are diagram references rather than effect geometry. This settles fake-3D sprites as sufficient for the placeholder phase and deliberately postpones real 3D and bespoke spell art. |
