...
## Canvas stretch makes UI text non-pixel-exact at most window sizes
...
Measured in the VFX debug scene's text specimen, which now reports the live
factor: 1152 x 648 gives x1.000 and perfectly uniform strokes; 1340 x 754 gives
x1.163 and visibly uneven ones. It degrades XenoText the same way — a bitmap
face only makes it easier to see.

Three ways out, none free, and the choice is a display decision rather than a
typography one:

- `stretch/scale_mode = "integer"` keeps the current model and guarantees whole
  factors, but letterboxes hard: 1920 x 1080 against a 1152 base scales to x1
  and leaves wide bars, because x2 does not fit.
- `stretch/mode = "disabled"` renders UI 1:1 at the native window size and is
  always exact. This is probably closest to the intended model already — the
  world has its own SubViewport in `RetroRenderController` and window geometry
  in `NoggTheme` is authored in absolute pixels — but UI then occupies a
  smaller share of a large screen unless something picks a scale.
- Author `viewport_width`/`viewport_height` at a base the target resolutions are
  whole multiples of. Only helps at those exact sizes.

The second option pairs with the open question of a resolution-aware integer
scale for the UI face, since Nogg Terminal renders only at whole multiples of
12 and something has to choose between x2 and x3 per screen.
