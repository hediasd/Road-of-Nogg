## MagentaReductionEffect over a hex footprint. Owned by HXB-11; the donor is not modified.
##
## A SUBCLASS RATHER THAN A COPY. GDScript methods are virtual, so overriding the one function
## carrying the square assumption inherits every other line of MagentaReductionEffect unchanged -- its
...
##
## Its ground wash is then replaced outright with the footprint's own silhouette at its exact
## extent, so the cells a player reads off the floor are the cells the resolver actually produced.

class_name HexMagentaReductionEffect
extends MagentaReductionEffect
...
static func createHexPlayback(
	...
	var playback := HexMagentaReductionEffect.new()
	playback.name = "HexMagentaReductionEffect"
...
func setFootprint(radius: int, groundSpan: float, areaShape: String = "circle") -> void:
	...
	if _hexFootprint == null or _hexFootprint.isEmpty():
		return
	HexVfxFootprintAdapterScript.applyGroundWash(_groundMesh, _groundMaterial, _hexFootprint)
