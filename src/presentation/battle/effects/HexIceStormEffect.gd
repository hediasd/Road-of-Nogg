## IceStormEffect over a hex footprint. Owned by HXB-11; the donor is not modified.
##
## A SUBCLASS RATHER THAN A COPY. GDScript methods are virtual, so overriding the one function
## carrying the square assumption inherits every other line of IceStormEffect unchanged -- its
...
class_name HexIceStormEffect
extends IceStormEffect
...
static func createHexPlayback(
	...
	var playback := HexIceStormEffect.new()
	playback.name = "HexIceStormEffect"
