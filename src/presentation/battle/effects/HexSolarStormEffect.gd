## SolarStormEffect over a hex footprint. Owned by HXB-11; the donor is not modified.
##
## A SUBCLASS RATHER THAN A COPY. GDScript methods are virtual, so overriding the one function
## carrying the square assumption inherits every other line of SolarStormEffect unchanged -- its
...
class_name HexSolarStormEffect
extends SolarStormEffect
...
static func createHexPlayback(
	...
	var playback := HexSolarStormEffect.new()
	playback.name = "HexSolarStormEffect"
