## FireStormEffect over a hex footprint. Owned by HXB-11; the donor is not modified.
##
## A SUBCLASS RATHER THAN A COPY. GDScript methods are virtual, so overriding the one function
## carrying the square assumption inherits every other line of FireStormEffect unchanged -- its
...
class_name HexFireStormEffect
extends FireStormEffect
...
static func createHexPlayback(
	...
	var playback := HexFireStormEffect.new()
	playback.name = "HexFireStormEffect"
