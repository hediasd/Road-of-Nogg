...
## The empty influence map every walkable tile starts from. Split out of
## generate() so a caller that must not spend a whole map's worth of work in one
## frame can build it enemy by enemy — see CommandDeliberation and
## docs/ARCHITECTURE.md, "Frame budget: deliberation must not block
## presentation". accumulateEnemy() over threateningEnemies() is exactly what
## generate() does, so both paths produce the same map.
static func beginMap(state: BattleState) -> Dictionary:
	var map: Dictionary = {}
...
## Alive monsters hostile to `team`, in the order generate() would visit them.
static func threateningEnemies(state: BattleState, team: int) -> Array:
	var enemies: Array = []
...
## Adds one enemy's contribution to `map`, in place. The unit of work a
## time-sliced caller can afford between frames.
static func accumulateEnemy(
		state: BattleState,
		map: Dictionary,
		enemyID: int,
