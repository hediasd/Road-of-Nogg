...
const SessionPanelScript = preload("res://src/presentation/battle/ui/HexBattleSessionPanel.gd")
...
func _checkResultAndSessionModels() -> void:
	...
	var paused := HudScript.sessionModel({"phase": "battle", "paused": true, "speed": 0.5,
		"can_skip": false})
	var rows: Array = paused["rows"]
	_require(str(paused["title"]) == "Paused" and str(rows[0]["label"]) == "Resume"
		and str(rows[0]["value"]) == "P", "a paused session did not offer Resume on P")
	_require(str(rows[1]["label"]) == "Speed 0.5x" and str(rows[1]["value"]) == "F",
		"the speed row read '%s'" % rows[1]["label"])
	_require(not bool(rows[2]["enabled"]) and str(rows[2]["value"]) == "Enter",
		"Skip was offered with nothing to skip")
	var complete := HudScript.sessionModel({"phase": "complete", "result": "Draw", "rounds": 7})
	var ids := []
	for row in complete["rows"]:
		ids.append(str(row["id"]))
	_require(str(complete["title"]) == "Draw" and ids.has(SessionPanelScript.RESTART)
		and ids.has(SessionPanelScript.SETUP) and not ids.has(SessionPanelScript.PAUSE),
...
func _checkCpuPauseSpeedSkip() -> void:
	...
	var session = _controller.hud.sessionPanel.drawnModel()
	_require(session is Dictionary and str(session.get("title", "")) == "Paused",
		"the session panel did not say Paused")
	...
	_controller.hud.session_command.emit(SessionPanelScript.SKIP)
	await _frames(3)
	...
	# Resume, then speed.
	_controller.hud.session_command.emit(SessionPanelScript.PAUSE)
	await _frames(1)
...
func _checkSessionLayout() -> void:
	...
	var session := Rect2(hud.sessionPanel.position, hud.sessionPanel.windowSize())
	_require(hud.sessionPanel._window.visible and session.size.y > 0.0,
		"the session window was not shown on a player's menu")
	_require(Rect2(Vector2.ZERO, Vector2(SCREEN)).encloses(session),
		"the session window %s leaves the 1280x720 screen" % session)
	var others := {
	...
	for window in [hud.inspection.actorWindow, hud.inspection.targetWindow, hud.inspection.forecastWindow]:
		others[window.name] = Rect2(window.position, Vector2(window.size.x, window.custom_minimum_size.y))
	for name in others:
		_require(not session.intersects(others[name]),
			"the session window overlaps %s at 1280x720" % name)
	_evidence["session_rect_720p"] = str(session)
...
func _checkPlayerPause() -> void:
	...
	await _frames(2)
	_require(not _controller.hud.sessionPanel._window.visible,
		"the session window stayed over the board during an aim")
	_pushKey(KEY_ESCAPE)
	await _frames(2)
	_require(_controller.hud.sessionPanel._window.visible,
		"the session window did not come back when the aim ended")
	_controller.returnToSetup()
	await _frames(2)
...
func _checkLedgerFinalDrainAndRestart() -> void:
	...
	await _frames(2)
	var session = _controller.hud.sessionPanel.drawnModel()
	_require(session is Dictionary and str(session.get("title", "")) == result,
		"the session panel did not show the result")
